...
import { X, RotateCcw, Hash, Mail, User } from "lucide-react";
...
export function UsersFilters({ query, onChange, onReset }: UsersFiltersProps) {
  const hasAnyFilter =
    ...
    <div className="rounded-2xl border border-border bg-card p-5 flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <select
          ...
          onChange={(e) =>
            onChange({
              sortField: e.target.value as typeof UserSortField[number],
            })
          }
        >
          {UserSortField.map((f) => (
            <option key={f} value={f}>
              {f === "createdAt" ? "Date created" : capitalize(f)}
            </option>
          ))}
        </select>
      </div>
...
function TagInput({
  ...
  function commit() {
    const v = input.trim();
    if (!v) return;
    if (validateInput && !validateInput(v)) {
      setInvalid(true);
      return;
    }
